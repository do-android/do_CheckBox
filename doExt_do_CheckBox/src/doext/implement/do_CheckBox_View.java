...
import doext.define.do_CheckBox_IMethod;
import doext.define.do_CheckBox_MAbstract;

/**
 * 自定义扩展UIView组件实现类，此类必须继承相应VIEW类，并实现DoIUIModuleView,do_CheckBox_IMethod接口；
 * #如何调用组件自定义事件？可以通过如下方法触发事件：
 * this.model.getEventCenter().fireEvent(_messageName, jsonResult);
 * 参数解释：@_messageName字符串事件名称，@jsonResult传递事件参数对象； 获取DoInvokeResult对象方式new
 * DoInvokeResult(this.model.getUniqueKey());
 */
public class do_CheckBox_View extends CheckBox implements DoIUIModuleView, do_CheckBox_IMethod, OnCheckedChangeListener {

	/**
	 * 每个UIview都会引用一个具体的model实例；
	 */
	private do_CheckBox_MAbstract model;

	public do_CheckBox_View(Context context) {
		super(context);
	}
	...
	public void onPropertiesChanged(Map<String, String> _changedValues) {
		...
		if (_changedValues.containsKey("enabled")) {
			this.setEnabled(DoTextHelper.strToBool(_changedValues.get("enabled"), true));
		}
	}
	...
	private void doCheckBoxView_changed(boolean isChecked) {
		...
		this.model.getEventCenter().fireEvent("checkChanged", _invokeResult);
	}
